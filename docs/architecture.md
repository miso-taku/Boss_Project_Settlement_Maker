...
- **レスポンス**（200）
  - `drafts`: 返信案のリスト（要素数は 1 件。各要素は `{ "text": string }`）
...
| **チェック** | 返信案チェックエージェント | 返信案リスト＋依頼文・自分の状況 | チェック結果（3項目を10点満点で評価、全て8以上で合格。指摘・改善点） |
| **作り直し** | 返信案作り直しエージェント | 返信案リスト＋チェック指摘＋依頼文・自分の状況 | 返信案リスト（修正版・1件） |

**フロー**

1. **生成**: 依頼文＋自分の状況 → 返信案生成エージェント → 返信案リスト（初稿）
2. **チェック**: 返信案リスト＋依頼文・状況 → 返信案チェックエージェント → チェック結果（3項目を10点満点で評価。全て8以上でOK、1つでも8未満ならNG＋指摘・改善点）
